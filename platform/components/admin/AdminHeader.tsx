...
import { getBreadcrumb } from '@/components/layout/AdminSidebar';
...
export function AdminHeader({
  ...
  const matches = useMemo(
    () => (query.trim() ? JUMP_TARGETS.filter((t) => t.label.toLowerCase().includes(query.trim().toLowerCase())) : []),
    [query],
  );
      ...
      <div className="relative shrink-0" ref={notifRef}>
        ...
        {notifOpen && (
          <div className="absolute right-0 top-full z-30 mt-2 w-64 rounded-xl border border-sand-200 bg-surface p-3 shadow-soft">
            ...
            <p className="mt-2 text-sm text-ink-500">You&apos;re all caught up — nothing new to review.</p>
          </div>
        )}
      </div>
      ...
      <div className="relative shrink-0" ref={menuRef}>
        ...
        >
          ...
          <span className="hidden flex-col items-start leading-tight lg:flex">
            <span className="max-w-[8rem] truncate">{user.name ?? user.email ?? 'Admin'}</span>
