...
// Generic building block behind both the Admin and Moderator "work" shells
// (sidebar + grouped nav + collapsible/mobile-drawer behavior). Each role's
// sidebar (components/layout/AdminSidebar.tsx, ModeratorSidebar.tsx) is a
// thin wrapper supplying its own nav tree, brand icon, and panel label — the
// actual rendering/interaction logic lives here once.
export type Icon = ComponentType<SVGProps<SVGSVGElement>>;
export type NavLeaf = { href: string; label: string; icon: Icon; exact?: boolean; soon?: boolean };
export type NavGroup = { label: string; items: NavLeaf[] };

export function getNavBreadcrumb(
  nav: NavGroup[],
  pathname: string | null,
  fallback: { group: string; label: string },
    ...
    >
      ...
      {!collapsed && (
        <span className="min-w-0">
          ...
          <span className="block truncate text-xs text-ink-500">{panelLabel}</span>
        </span>
      )}
    </Link>
  );
